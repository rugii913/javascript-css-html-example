...
### \[예제 실습\] 프레임과 그룹을 적용했을 때의 차이점 체험하기 - 앞 내용 재정리
- 배경색(fill)
  - frame은 fill을 줄 수 있음
  - group은 fill을 줄 수 없음
    - fill이 있는 것처럼 보이려면 별도로 group을 꽉 채우는 retangle, frame 같은 것을 두어 배경으로 보이게 해야 하므로 비효율적
    - group의 fill을 바꾸면 모든 직속 하위 요소들의 fill이 변경됨
- 사이즈 조절
  - frame의 사이즈를 조절해도 직속 하위 요소들의 사이즈가 변하지는 않음
  - group의 사이즈를 조절하면 직속 하위 요소들의 사이즈가 모두 변함
    - 심지어 직속 하위 요소들에 constraints를 설정해도 영역으로써 기능이 없으므로 하위 요소들이 고정되지 않음
- 특별한 이유가 없다면 UI를 만들 때 group이 아닌 frame으로 제작하는 편이 나음

### 그리기: 프레임에 이미지 넣기 - 하위 레이어 vs. fill
- (방법 1) 상단 툴바 shape tool의 Place image... 활용(단축키 ctrl + shift + k)
  - place image 툴 시작 후 파일 시스템에서 이미지 파일 열기
  - 마우스 포인터가 십자 모양이 되면 frame 안 영역을 클릭
    - 이미지가 frame의 하위 요소로 들어감
    - 이미지를 선택한 상태에서 design panel에서 이미지 크기를 적절하게 조절 후 가운데 정렬(alt + v, alt + h)
    - bounding box 모서리를 드래그해서 이미지 크기를 적절하게 조절
      - 다른 요소와 마찬가지로 alt를 누른 상태로 드래그하면 중심이 유지됨
    - (cf.) 이미지 layer 안의 사진은 기본적으로 비율이 유지됨
- (방법 2) design panel의 fill 활용
  - frame을 선택한 상태에서 fill 추가 혹은 원래 존재하는 fill의 색 클릭
    - 컬러 팔레트 위의 Image 버튼을 클릭
    - Choose image 클릭 후 파일 시스템에서 이미지 파일 열기
    - 어느 정도 frame 사이즈에 맞게 이미지가 들어감
    - frame과 이미지와의 관계 옵션을 Fill, Fit, Crop, Tile 중 선택(기본 설정 Fill)
  - (cf.) 파일 시스템에서 여는 대신 이미지 파일을 복사한 후 Choose image 쪽에 붙여넣는 방법도 가능
- 두 방법의 차이점
  - 하위 레이어 여부, 사이즈 및 위치 조절 가능 여부에서 차이가 있음
  - 방법 1의 경우 frame 안에 image가 객체로 들어감(하위 레이어), 방법 2의 경우 frame 자체의 fill이 됨(이미지는 하위 레이어와 관계 없음)
